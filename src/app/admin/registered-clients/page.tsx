  ...
  const fetchData = async () => {
    ...
    try {
      ...
      try {
        ...
        snap.forEach(doc => {
          const data = doc.data();
          if (data && data.clientId) sourcingIds.add(data.clientId);
        });
      } catch (e) {}
    ...
    <div className="container py-8 space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Gestion des accès et dossiers clients.</p>
        </div>
        <Button onClick={() => setIsLeadsDialogOpen(true)} className="bg-primary hover:bg-primary/90 font-bold"><UserPlus className="mr-2 h-4 w-4" /> Nouveau Prospect</Button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-1 border-primary/20 bg-primary/5">
          <CardHeader className="pb-3"><CardTitle className="text-sm font-bold flex items-center gap-2"><Euro className="h-4 w-4 text-primary" /> TAUX DE CHANGE</CardTitle></CardHeader>
          <CardContent><div className="flex gap-2"><Input type="number" step="0.0001" value={localRate} onChange={(e) => setLocalRate(e.target.value)} /><Button onClick={() => currencyContext?.setExchangeRate(parseFloat(localRate))}>FIXER</Button></div></CardContent>
        </Card>
