...
import { CompanyInfoProviderWrapper } from './company-info-provider-wrapper';
...
async function QuotePreview({ quote, customer }: { quote: Quote, customer: Customer | null }) {
    const commissionAmount = (quote.subTotal + (quote.transportCost || 0)) * ((quote.commissionRate || 0) / 100);

    return (
        <CompanyInfoProviderWrapper>
            {(companyInfo) => (
